...
const setupMainPackageWatcher = ({ config: { server } }) => {
  ...
  return getWatcher({
    ...
    configFile: "build-scripts/main.vite.config.ts",
    writeBundle() {
...
const setupPreloadPackageWatcher = ({ ws }) =>
  getWatcher({
    ...
    configFile: "build-scripts/preload.vite.config.ts",
    writeBundle() {
      // Generating exposedInMainWorld.d.ts when preload package is changed.
      generateAsync({
        input: "electron-preload/**/*.ts",
        output: "electron-preload/exposedInMainWorld.d.ts",
      });
...
(async () => {
  try {
    const viteDevServer = await createServer({
      ...sharedConfig,
      configFile: "build-scripts/renderer.vite.config.ts",
    });
